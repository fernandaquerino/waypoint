    ...
    <main className="min-h-screen px-5 py-6 sm:px-8 lg:px-10">
      <div className="mx-auto flex min-h-[calc(100vh-48px)] w-full max-w-5xl flex-col">
        <header className="flex items-center justify-between gap-4 border-b border-border pb-5">
          <Link className="rounded-control text-lg font-semibold outline-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--ring)]" href="/dashboard">
            Waypoint
          </Link>
        ...
        <section className="grid flex-1 gap-6 py-8 lg:grid-cols-[minmax(0,1fr)_320px]">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-muted">
              Este espaço já nasce autenticado para que as próximas features usem
              o `userId` vindo da sessão, não do client.
            </p>
          </div>
          ...
          <aside className="rounded-card border border-border bg-surface p-5 shadow-[var(--shadow-soft)]">
            ...
            <dl className="mt-5 space-y-4 text-sm">
              <div className="border-t border-border pt-4">
                ...
                <dd className="mt-1 font-medium text-foreground">{privateApi.message}</dd>
              </div>
              <div className="border-t border-border pt-4">
